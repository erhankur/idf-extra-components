...
#define traceTASK_PRIORITY_DISINHERIT( pxTCB, uxBasePriority ) esp_trace_ctf_task_priority_disinherit((uint32_t)pxTCB)
...
#define traceTASK_CREATE( pxNewTCB ) do { \
                                        if (pxNewTCB != NULL) { \
                                            esp_trace_ctf_task_create((uint32_t)pxNewTCB, (uint32_t)pxNewTCB->uxPriority, pxNewTCB->pcTaskName); \
                                        } \
                                    } while (0)

#define traceTASK_PRIORITY_SET(pxTask, uxNewPriority) esp_trace_ctf_task_priority_set((uint32_t)pxTask, uxNewPriority)
/* SMP IDLE tasks are unpinned, so detect them by name there */
#if ( INCLUDE_xTaskGetIdleTaskHandle == 1  && !defined(CONFIG_FREERTOS_SMP))
#define traceTASK_SWITCHED_IN()     do { \
                                    if (prvGetTCBFromHandle(NULL) == xTaskGetIdleTaskHandle()) { \
                                        esp_trace_ctf_idle(); \
                                    } else { \
...
#define traceISR_EXIT_TO_SCHEDULER() esp_trace_ctf_isr_exit_to_scheduler()
